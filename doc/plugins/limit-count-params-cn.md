...
|key           |必选     |是用来做请求计数的依据，使用当前的uri query。|
|group         |可选     |分组的标识，当使用redis作为计数器存储时，为了让多个route共享限速使用。这里只要将共享的route使用相同的group，就可以让他们使用相同的计数器。如果不填，则全部使用一样的计数器。 |
...
|redis.database|可选     |当使用 `redis` 限速策略时，该属性是 Redis 服务节点的database，默认 0。|
